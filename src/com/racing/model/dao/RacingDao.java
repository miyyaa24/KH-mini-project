	...
	public List<Racing> selectByHrName(Connection conn, String hrName) {
		...
		try {
			...
			while (rs.next()) {
				...
				Racing info = null;	info = new Racing(RNO, rcDate, rcNo, rcDist, rcOrd, rcTime, rcChul, hrno, hrName2, wgHr, jkNo, jkName,
						chaksun, weath);
				list.add(info);
			}
	...
	public List<Racing> selectByJkName(Connection conn, String jkName) {
		...
		try {
			...
			while (rs.next()) {
				...
				Racing info = new Racing(RNO, rcDate, rcNo, rcDist, rcOrd, rcTime, rcChul, hrno, hrName, wgHr, jkNo, jkName2,
						chaksun, weath);
				list.add(info);
			}
		} catch (Exception e) {
